...
interface Props {
  ...
  showIplIncome?: boolean;
  showUmkmAdsIncome?: boolean;
}
...
export function FinancialTrendChart({ data, showTotalBilled = true, showIplIncome = true, showUmkmAdsIncome = true }: Readonly<Props>) {
  return <div className="w-full overflow-x-auto pb-2"><div className="h-80 min-w-[640px]">
    <ResponsiveContainer width="100%" height="100%">
      <ComposedChart data={data} margin={{ top: 12, right: 20, left: 8, bottom: 0 }}>
        ...
        {showIplIncome ? <Bar dataKey="ipl_income" name="IPL" stackId="income" fill="#0f766e" maxBarSize={38} /> : null}
        {showUmkmAdsIncome ? <Bar dataKey="umkm_ads_income" name="Iklan UMKM" stackId="income" fill="#f59e0b" radius={[3, 3, 0, 0]} maxBarSize={38} /> : null}
        ...
        <Line dataKey="total_income" name="Total Pendapatan" type="monotone" stroke="#2563eb" strokeWidth={2} dot={{ r: 3 }} activeDot={{ r: 5 }} />
      </ComposedChart>
    </ResponsiveContainer>
  </div></div>;
}
