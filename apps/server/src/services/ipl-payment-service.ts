...
const isResident = () => residentRoles.has((getCurrentAuth()?.role_code || "").toUpperCase());
...
const getUserFamilyId = async (client: PoolClient | typeof pool, userId?: string | null) => {
...
const decodeProof = (request: IplPaymentInterface) => {
  ...
  if (!buffer.length || buffer.length > MAX_FILE_SIZE) throw new Error("Ukuran bukti pembayaran maksimal 5 MB");
  ...
  if (!isJpeg && !isPng && !isPdf) throw new Error("Bukti hanya boleh berupa JPEG, PNG, atau PDF");
...
const saveProof = async (request: IplPaymentInterface) => {
  ...
  const now = new Date();
  const relativeDirectory = path.join(String(now.getFullYear()), String(now.getMonth() + 1).padStart(2, "0"));
  const directory = path.join(storageRoot, relativeDirectory);
  await mkdir(directory, { recursive: true });
  const relativePath = path.join(relativeDirectory, `${randomUUID()}.${proof.extension}`);
  await writeFile(path.join(storageRoot, relativePath), proof.buffer, { flag: "wx" });
  return { relativePath, mime: proof.mime, size: proof.buffer.length };
};

const notifyFamily = async (client: PoolClient, familyId: string, type: string, title: string, message: string, referenceId: string) => {
  await client.query(
    `INSERT INTO ${tableNames.notification}
       (user_id, type, title, message, reference_id, reference_url, created_by_id)
     SELECT DISTINCT u.id, $2, $3, $4, $5::uuid, '/operation/tagihan', $6::uuid
    ...
    [familyId, type, title, message, referenceId, getCurrentAuth()?.user_id || null],
  );
};

const notifyAdmins = async (client: PoolClient, paymentId: string, billNumber: string) => {
  await client.query(
    `INSERT INTO ${tableNames.notification}
       (user_id, type, title, message, reference_id, reference_url, created_by_id)
...
const applyApprovedPayment = async (client: PoolClient, paymentId: string) => {
  const paymentResult = await client.query<IplPaymentInterface>(
    `SELECT * FROM ${tableNames.iplPayment} WHERE id = $1 AND is_deleted = false FOR UPDATE`, [paymentId],
  );
  const payment = paymentResult.rows[0];
  if (!payment?.bill_id || !payment.family_id) throw new Error("Pembayaran tidak ditemukan");
  const billResult = await client.query<{ id: string; amount: string; paid_amount: string; credit_amount: string; status: string; bill_number: string }>(
    `SELECT * FROM ${tableNames.iplBill} WHERE id = $1 AND status <> 'CANCELLED' AND is_deleted = false FOR UPDATE`, [payment.bill_id],
  );
  ...
  const nextStatus = nextCredit > 0 ? "OVERPAID" : nextPaid >= Number(bill.amount) ? "PAID" : nextPaid > 0 ? "PARTIALLY_PAID" : "UNPAID";
...
export const createPaymentService = async (request: IplPaymentInterface) => {
  ...
  if (!request.bill_id || !Number.isFinite(amount) || amount <= 0 || !request.payment_date || !request.payment_method) {
    return { status: 400, message: "Data pembayaran belum lengkap", data: null };
  }
  ...
  try {
    ...
    const billResult = await client.query<{ id: string; family_id: string; bill_number: string; status: string; amount: string; paid_amount: string }>(
    ...
    if (!admin) {
      ...
      if (!familyId || familyId !== bill.family_id) throw new Error("Anda tidak memiliki akses ke tagihan ini");
    ...
    const inserted = await client.query<IplPaymentInterface>(
      ...
      [bill.id, bill.family_id, amount, request.payment_date, request.payment_method,
       request.reference_number?.trim() || null, request.note?.trim() || null, saved.relativePath,
       path.basename(request.proof_original_name || `bukti.${saved.mime.split("/")[1]}`), saved.mime, saved.size,
       status, admin ? "ADMIN" : "WARGA", auth?.user_id || null],
    );
    const payment = inserted.rows[0]!;
    if (admin) {
      await applyApprovedPayment(client, payment.id!);
      await notifyFamily(client, bill.family_id, "IPL_PAYMENT_APPROVED", "Pembayaran IPL tercatat", `Pembayaran ${bill.bill_number} telah dicatat oleh admin.`, payment.id!);
    } else {
      await notifyAdmins(client, payment.id!, bill.bill_number);
    }
    await client.query("COMMIT");
    return { status: 201, message: admin ? "Pembayaran berhasil dicatat" : "Bukti pembayaran berhasil diajukan", data: payment };
  } catch (error) {
    await client.query("ROLLBACK");
    if (saved) await unlink(path.join(storageRoot, saved.relativePath)).catch(() => undefined);
    const message = error instanceof Error ? error.message : "Gagal menyimpan pembayaran";
...
const loadPayments = async (request: BaseRequest<IplPaymentInterface>, familyId?: string | null) => {
  const { page, pageSize, offset } = paging(request);
  const params = request.params || {};
  const values: unknown[] = [];
  const where = ["p.is_deleted = false"];
  if (familyId) { values.push(familyId); where.push(`p.family_id = $${values.length}`); }
  if (params.status) { values.push(params.status); where.push(`p.status = $${values.length}`); }
  if (params.payment_method) { values.push(params.payment_method); where.push(`p.payment_method = $${values.length}`); }
  ...
  const count = await pool.query(`SELECT COUNT(*) FROM ${tableNames.iplPayment} p ${joins} ${clause}`, values);
  ...
  const data = await pool.query<IplPaymentInterface>(
    `SELECT p.*, bill.bill_number, bill.period, f.no_kk AS family_no_kk, f.address AS family_address
     ...
     ORDER BY p.created_time DESC LIMIT $${values.length - 1} OFFSET $${values.length}`, values,
  );
  return { status: 200, message: "Request successful", data: data.rows, metaData: { total: Number(count.rows[0]?.count || 0), page, pageSize } };
};

export const loadPaymentService = (request: BaseRequest<IplPaymentInterface>) => {
  if (isResident()) return { status: 403, message: "Akses ditolak", data: [] };
  return loadPayments(request);
};

export const loadMyPaymentService = async (request: BaseRequest<IplPaymentInterface>) => {
  const familyId = await getUserFamilyId(pool, getCurrentAuth()?.user_id);
  return familyId ? loadPayments(request, familyId) : { status: 200, message: "Akun belum terhubung dengan keluarga", data: [], metaData: { total: 0, page: 1, pageSize: 10 } };
};

export const approvePaymentService = async (id?: string | null) => {
  if (isResident()) return { status: 403, message: "Akses ditolak", data: null };
  const client = await pool.connect();
  try {
    await client.query("BEGIN");
    const check = await client.query<IplPaymentInterface>(`SELECT * FROM ${tableNames.iplPayment} WHERE id = $1 AND status = 'PENDING' FOR UPDATE`, [id]);
    if (!check.rows[0]) throw new Error("Pembayaran tidak dapat disetujui");
    const applied = await applyApprovedPayment(client, id!);
    await notifyFamily(client, applied.payment.family_id!, "IPL_PAYMENT_APPROVED", "Pembayaran disetujui", `Pembayaran ${applied.billNumber} telah disetujui.`, id!);
    await client.query("COMMIT");
    return { status: 200, message: "Pembayaran berhasil disetujui", data: null };
  } catch (error) {
    await client.query("ROLLBACK");
    return { status: 409, message: error instanceof Error ? error.message : "Gagal menyetujui pembayaran", data: null };
  } finally { client.release(); }
};

export const rejectPaymentService = async (id?: string | null, note?: string | null) => {
  if (isResident()) return { status: 403, message: "Akses ditolak", data: null };
  if (!note?.trim()) return { status: 400, message: "Alasan penolakan wajib diisi", data: null };
  const client = await pool.connect();
  try {
    await client.query("BEGIN");
    const result = await client.query<IplPaymentInterface>(
      `UPDATE ${tableNames.iplPayment} SET status = 'REJECTED', rejection_note = $2,
       ...
       WHERE id = $1 AND status = 'PENDING' RETURNING *`, [id, note.trim(), getCurrentAuth()?.user_id || null],
    );
    const payment = result.rows[0];
    if (!payment?.family_id) throw new Error("Pembayaran tidak dapat ditolak");
    await notifyFamily(client, payment.family_id, "IPL_PAYMENT_REJECTED", "Pembayaran ditolak", `Bukti pembayaran ditolak: ${note.trim()}`, payment.id!);
    await client.query("COMMIT");
    return { status: 200, message: "Pembayaran berhasil ditolak", data: null };
  } catch (error) {
    await client.query("ROLLBACK");
    return { status: 409, message: error instanceof Error ? error.message : "Gagal menolak pembayaran", data: null };
  } finally { client.release(); }
};

export const reversePaymentService = async (id?: string | null, note?: string | null) => {
  if (isResident()) return { status: 403, message: "Akses ditolak", data: null };
  if (!note?.trim()) return { status: 400, message: "Alasan reversal wajib diisi", data: null };
  const client = await pool.connect();
  try {
    await client.query("BEGIN");
    const paymentResult = await client.query<IplPaymentInterface>(`SELECT * FROM ${tableNames.iplPayment} WHERE id = $1 AND status = 'APPROVED' FOR UPDATE`, [id]);
    const payment = paymentResult.rows[0];
    if (!payment?.bill_id || !payment.family_id) throw new Error("Pembayaran tidak dapat dibatalkan");
    const billResult = await client.query<{ amount: string; paid_amount: string; credit_amount: string }>(`SELECT amount, paid_amount, credit_amount FROM ${tableNames.iplBill} WHERE id = $1 FOR UPDATE`, [payment.bill_id]);
    const bill = billResult.rows[0];
    if (!bill) throw new Error("Tagihan tidak ditemukan");
    const nextPaid = Math.max(Number(bill.paid_amount) - Number(payment.allocated_amount || 0), 0);
    const nextCredit = Math.max(Number(bill.credit_amount) - Number(payment.credit_amount || 0), 0);
    const nextStatus = nextCredit > 0 ? "OVERPAID" : nextPaid >= Number(bill.amount) ? "PAID" : nextPaid > 0 ? "PARTIALLY_PAID" : "UNPAID";
    ...
    await notifyFamily(client, payment.family_id, "IPL_PAYMENT_REVERSED", "Pembayaran dibatalkan", `Pembayaran dibatalkan oleh admin: ${note.trim()}`, payment.id!);
    await client.query("COMMIT");
    return { status: 200, message: "Pembayaran berhasil direversal", data: null };
  } catch (error) {
    await client.query("ROLLBACK");
    return { status: 409, message: error instanceof Error ? error.message : "Gagal melakukan reversal", data: null };
  } finally { client.release(); }
};

export const getPaymentProofService = async (id?: string | null) => {
  const result = await pool.query<IplPaymentInterface & { proof_path: string }>(`SELECT * FROM ${tableNames.iplPayment} WHERE id = $1 AND is_deleted = false`, [id]);
  const payment = result.rows[0];
  ...
  return { path: absolutePath, name: payment.proof_original_name || "bukti", mime: payment.proof_mime_type || "application/octet-stream" };
};
