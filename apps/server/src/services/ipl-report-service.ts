...
const isAdmin = () => !["WARGA", "WRG"].includes((getCurrentAuth()?.role_code || "").toUpperCase());

export const getIplReportSummaryService = async (request: IplReportRequest) => {
  if (!isAdmin()) return { status: 403, message: "Akses ditolak", data: null };
    ...
    `WITH filtered_bills AS (
     ...
     SELECT
       ...
       COALESCE((SELECT SUM(credit.balance) FROM ${tableNames.iplFamilyCredit} credit
         ...
         WHERE ($2::text IS NULL OR family.no_kk ILIKE '%' || $2 || '%')), 0)::float8 AS family_credit_balance,
...
export const loadCreditLedgerService = async (request: BaseRequest<IplCreditLedgerInterface>) => {
  if (!isAdmin()) return { status: 403, message: "Akses ditolak", data: [] };
