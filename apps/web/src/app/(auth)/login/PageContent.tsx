...
export default function PageContent() {
    ...
    const firstMenuUrl = (menu: AuthLoginResponse["menu"]) => {
        const root = menu[0];
        if (!root) return "/403";
        const child = root.child?.[0];
        return child ? `/${root.pathUrl}/${child.pathUrl}` : `/${root.pathUrl}`;
    };
        ...
        await callApi(
            ...
            },
            {
                onSuccess(response: BaseResponse<AuthLoginResponse | null>) {
                    ...
                    const hasDashboard = data.menu.some((item) => item.pathUrl === "dashboard");
                    router.push(callbackUrl === "/dashboard" && !hasDashboard ? firstMenuUrl(data.menu) : callbackUrl);
                },
