...
import MonthYearPicker, {
  type YearMonth,
} from "@monorepo/ui/components/monthyear";
import {
  Popover,
  PopoverContent,
  PopoverTrigger,
...
const toPeriod = ({ month, year }: YearMonth) =>
  `${monthCodes[month - 1] || "JAN"}-${year}`;
...
export default function PageContent() {
  ...
  const canAdd = useMemo(
    () => canAccessRoute(authMenu, "ipl/generate-bill", "ADD"),
    [authMenu],
  );
  const canEdit = useMemo(
    () => canAccessRoute(authMenu, "ipl/generate-bill", "EDIT"),
    [authMenu],
  );
  const canAction = useMemo(
    () => canAccessRoute(authMenu, "ipl/generate-bill", "ACTION"),
    [authMenu],
  );
  ...
  const [confirmation, setConfirmation] = useState<{
    type: "PUBLISH" | "CANCEL";
    batch: IplBillBatchInterface;
  } | null>(null);
  const [editing, setEditing] = useState<IplBillBatchInterface | null>(null);
  const [form, setForm] = useState({ period: "", amount: "", note: "" });
  const { callApi: loadBatch, loading: loadingBatch } =
    useApiService("loadIplBatch");
  const { callApi: loadBill, loading: loadingBill } =
    useApiService("loadIplBill");
  const { callApi: createBatch, loading: creating } =
    useApiService("insertIplBatch");
  const { callApi: updateBatch, loading: updating } =
    useApiService("updateIplBatch");
  const { callApi: publishBatch, loading: publishing } =
    useApiService("publishIplBatch");
  const { callApi: cancelBatch, loading: cancelling } =
    useApiService("cancelIplBatch");
      ...
      loadBatch(
        {
          params: {
            ...
            status: (filter.status ||
              undefined) as IplBillBatchInterface["status"],
          },
          metadata: meta,
        },
        {
      ...
      loadBill(
        {
          params: {
            period: filter.period || undefined,
            status: (filter.status || undefined) as IplBillInterface["status"],
          },
          metadata: meta,
        },
        {
  ...
  const openForm = (batch?: IplBillBatchInterface) => {
    ...
    setForm({
      period: batch?.period || currentPeriod,
      amount: batch?.amount ? String(batch.amount) : "",
      note: batch?.note || "",
    });
    setDialogOpen(true);
  };
  ...
  const save = async () => {
    ...
    await call(payload, {
      onSuccess: () => {
        toast.success(editing ? "Draft diperbarui" : "Draft dibuat");
        setDialogOpen(false);
        reload();
      },
  ...
  const publish = async (batch: IplBillBatchInterface) => {
    await publishBatch(
      { id: batch.id },
      {
        onSuccess: (response) => {
          toast.success(
            `${response.data?.generated || 0} tagihan dibuat, ${response.data?.skipped || 0} dilewati`,
          );
          setConfirmation(null);
          reload();
        },
        onError: (error) =>
          toast.error(error.message || "Gagal menerbitkan tagihan"),
      },
    );
  };
  ...
  return (
    <div className="mt-6 space-y-6">
      <div className="flex flex-col gap-4 border-b border-slate-200 pb-5 md:flex-row md:items-end md:justify-between">
        <div>
          <h1 className="text-2xl font-bold text-slate-900">IPL Billing</h1>
          <p className="mt-1 text-sm text-slate-500">
            Terbitkan dan pantau tagihan IPL keluarga berdomisili aktif.
          </p>
        </div>
        {canAdd && view === "batch" ? (
      ...
      <div className="inline-flex rounded-md border border-slate-200 bg-white p-1">
        <Button
          size="sm"
          variant={view === "batch" ? "default" : "ghost"}
          onClick={() => {
            setView("batch");
      ...
      <FilterPanel
        columns={2}
        onSubmit={() => setMeta((old) => ({ ...old, page: 1 }))}
        onReset={() => setFilter({ period: "", status: "" })}
      >
        <div>
          <Label>Periode</Label>
          <Input
        ...
        <div>
          ...
          <Input
            className="mt-2"
            placeholder={view === "batch" ? "DRAFT / PUBLISHED" : "UNPAID"}
            value={filter.status}
            onChange={(e) =>
              setFilter((old) => ({
                ...old,
                    ...
                    <div className="flex gap-1">
                      ...
                      {canAction && row.status === "DRAFT" ? (
                        <Button
                          ...
                          disabled={publishing}
                          onClick={() =>
                            setConfirmation({ type: "PUBLISH", batch: row })
                          }
                        >
                          <Rocket className="h-4 w-4" />
                        </Button>
                      ) : null}
                      {canAction && row.status !== "CANCELLED" ? (
                        <Button
                          size="icon"
                          variant="ghost"
                          title="Batalkan"
                          disabled={cancelling}
                          onClick={() =>
                            setConfirmation({ type: "CANCEL", batch: row })
                          }
                        >
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>
              {editing ? "Edit Batch Tagihan" : "Buat Draft Tagihan"}
            </DialogTitle>
            <DialogDescription>
              Tagihan individual baru dibuat saat draft diterbitkan.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
              ...
              <Popover>
                ...
                <PopoverContent align="start" className="w-auto p-0">
                  <MonthYearPicker
                    ...
                    onChange={(value) =>
                      setForm((old) => ({ ...old, period: toPeriod(value) }))
                    }
                  />
                </PopoverContent>
              </Popover>
            ...
            {editing?.eligible_family_count ? (
              <p className="flex items-center gap-2 text-sm text-slate-500">
                <ScrollText className="h-4 w-4" />{" "}
                {editing.eligible_family_count} keluarga memenuhi kriteria.
              </p>
            ) : null}
          </div>
          <DialogFooter>
            ...
            <Button
              disabled={creating || updating || !form.period || !form.amount}
              onClick={save}
            >
              Simpan Draft
            </Button>
      ...
      <SwalDialog
        ...
        confirmText={
          confirmation?.type === "PUBLISH" ? "Terbitkan" : "Batalkan Batch"
        }
        cancelText="Kembali"
        ...
        onConfirm={() =>
          confirmation &&
          (confirmation.type === "PUBLISH"
            ? publish(confirmation.batch)
            : cancel(confirmation.batch))
        }
      />
    </div>
  );
