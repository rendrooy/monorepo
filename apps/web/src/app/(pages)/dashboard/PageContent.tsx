...
const emptyDashboard: IplDashboardInterface = { audience: "RESIDENT", period: "", summary: { total_billed: 0, cash_received: 0, recognized_income: 0, outstanding_amount: 0, pending_payment_amount: 0, family_credit_balance: 0, total_bill_count: 0, paid_bill_count: 0 }, pending_payment_count: 0, recent_bills: [], recent_payments: [] };
...
export default function PageContent() {
  ...
  const [showCashReceived, setShowCashReceived] = useState(true);
  ...
  const metrics = resident ? [
    ...
    { label: "Total Tagihan", value: money(summary.total_billed), detail: `${summary.total_bill_count} tagihan`, icon: ReceiptText },
    { label: "Kas Diterima", value: money(summary.cash_received), detail: "Pembayaran approved", icon: Banknote },
    { label: "Pendapatan Teralokasi", value: money(summary.recognized_income), detail: `${paidPercentage}% tagihan lunas`, icon: Scale },
    { label: "Piutang", value: money(summary.outstanding_amount), detail: "Tagihan yang belum terbayar", icon: WalletCards },
    { label: "Payment Pending", value: money(summary.pending_payment_amount), detail: `${dashboard.pending_payment_count} menunggu verifikasi`, icon: Clock3 },
    { label: "Saldo Kredit", value: money(summary.family_credit_balance), detail: "Saldo seluruh keluarga", icon: CircleDollarSign },
  ];
  ...
  const incomeChange = previousTrend?.recognized_income
    ? ((Number(selectedTrend?.recognized_income || 0) - previousTrend.recognized_income) / previousTrend.recognized_income) * 100
    : null;
  ...
  return <div className="mt-6 space-y-6">
    ...
    <UmkmAdsCarousel />
    {!resident ? <Card><CardContent className="pt-6"><div className="flex flex-col gap-4 border-b border-slate-100 pb-4 md:flex-row md:items-start md:justify-between"><div><h2 className="font-semibold text-slate-900">Tren Pendapatan {period.split("-")[1]}</h2><p className="mt-1 text-sm text-slate-500">Realisasi pendapatan IPL dibanding total tagihan dan kas diterima.</p><div className="mt-3 flex flex-wrap gap-x-5 gap-y-2 text-sm"><span><span className="text-slate-500">Realisasi {period}:</span> <strong>{Number(selectedTrend?.realization_percentage || 0).toFixed(1)}%</strong></span><span><span className="text-slate-500">Perubahan:</span> <strong className={incomeChange !== null && incomeChange < 0 ? "text-red-600" : "text-emerald-600"}>{incomeChange === null ? "-" : `${incomeChange >= 0 ? "+" : ""}${incomeChange.toFixed(1)}%`}</strong></span></div></div><div className="inline-flex w-fit rounded-md border border-slate-200 p-1"><Button size="sm" variant={showTotalBilled ? "default" : "ghost"} onClick={() => setShowTotalBilled((value) => !value)}>Total Tagihan</Button><Button size="sm" variant={showCashReceived ? "default" : "ghost"} onClick={() => setShowCashReceived((value) => !value)}>Kas Diterima</Button></div></div>{loadingTrend ? <div className="mt-5 h-80 animate-pulse rounded-md bg-slate-100" /> : <div className="mt-5"><FinancialTrendChart data={trend} showTotalBilled={showTotalBilled} showCashReceived={showCashReceived} /></div>}</CardContent></Card> : null}
