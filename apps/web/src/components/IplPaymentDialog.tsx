...
export function IplPaymentDialog({
  open,
  bills,
  selectedBill,
  adminMode = false,
  ...
  useEffect(() => {
    ...
    setAmount(
      selectedBill?.remaining_amount
        ? String(selectedBill.remaining_amount)
        : "",
    );
  ...
  const bill = useMemo(
    () => bills.find((item) => item.id === billId) || selectedBill || null,
  ...
  const handleFile = (next?: File) => {
    ...
    if (!allowedTypes.has(next.type)) {
      toast.error("Bukti hanya boleh JPEG, PNG, atau PDF");
      return;
    }
  ...
  const submit = async () => {
    if (!bill?.id || !amount || Number(amount) <= 0 || !paymentDate || !file) {
      toast.error(
        "Tagihan, nominal, tanggal, dan bukti pembayaran wajib diisi",
      );
      return;
    }
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-xl">
        <DialogHeader>
          <DialogTitle>
            {adminMode ? "Catat Pembayaran Warga" : "Upload Bukti Pembayaran"}
          </DialogTitle>
          <DialogDescription>
            {adminMode
              ? "Pembayaran yang dicatat admin langsung disetujui."
              : "Pembayaran akan diperiksa oleh admin sebelum mengurangi tagihan."}
          </DialogDescription>
        </DialogHeader>
        <div className="space-y-4">
                ...
                <SelectContent>
                  {bills.map((item) => (
                    <SelectItem key={item.id} value={item.id!}>
                      {item.bill_number} - {item.family_no_kk} -{" "}
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <div className="space-y-2">
              <Label htmlFor="payment_amount">Nominal</Label>
              <Input
                id="payment_amount"
                type="number"
                min="1"
          ...
          <div className="space-y-2">
            ...
            <Select
              value={method}
              onValueChange={(value) => setMethod(value as IplPaymentMethod)}
            >
              <SelectTrigger>
                <SelectValue />
              </SelectTrigger>
          ...
          <div className="space-y-2">
            <Label htmlFor="payment_note">Catatan</Label>
            <Textarea
              id="payment_note"
              value={note}
          ...
          <div className="space-y-2">
            ...
            <label
              ...
              className="flex min-h-24 cursor-pointer items-center justify-center rounded-md border border-dashed border-slate-300 bg-slate-50 px-4 text-center hover:bg-slate-100"
            >
              {file ? (
                ...
                <span className="flex flex-col items-center gap-1 text-sm text-slate-500">
                  ...
                  Pilih JPEG, PNG, atau PDF (maks. 5 MB)
                </span>
              )}
            </label>
            <Input
